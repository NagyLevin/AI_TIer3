...
def control_to_cell(state: State, next_cell: tuple[int, int]) -> Tuple[int, int]:
    ...
    for ax in (-1, 0, 1):
        for ay in (-1, 0, 1):
            ...
            # Collision check
            if any(np.array_equal(new_pos_int, op) for op in other_positions): #ha valaki mar foglalja a helyet
                continue  # Skip

            # felmeres
            dist = float(np.linalg.norm(desired_pos - new_pos))  # Distance megnezem milyen messze van
            speed = float(np.linalg.norm(new_vel))               # megnézem milyen gyors vagyok
            ...
            acc_mag = abs(ax) + abs(ay) #stabilabb mozgas, azt valasztom, ahol kevesebbet kell ide oda mozogni nem kell rangatni a kormanyt
            
            # Weighted score
            score = dist + 0.5 * speed_penalty + 0.1 * acc_mag #sulyozom a lepest, minel kissebb suly annal jobb a lepes
    ...
    # Ha minden crash-elne -> Vészfék megallunk, lassitunk TODO fix mi van, ha rossz az algoritmus, akkor be tudunk szorulni, esetleg nezz ra erre
